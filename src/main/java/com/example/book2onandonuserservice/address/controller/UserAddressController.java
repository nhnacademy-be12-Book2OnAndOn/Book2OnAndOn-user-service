...
public class UserAddressController {
    ...
    @GetMapping
    public ResponseEntity<List<UserAddressResponseDto>> getMyAddresses(
    ...
    public ResponseEntity<UserAddressResponseDto> getAddressDetails(
            @RequestHeader(USER_ID_HEADER) Long userId,
            @PathVariable Long addressId
    ) {
        UserAddressResponseDto address = userAddressService.findByUserIdAndAddressId(userId, addressId);
    ...
    public ResponseEntity<UserAddressResponseDto> createAddress(
        ...
        UserAddressResponseDto responseDto = userAddressService.save(userId, request);
    ...
    public ResponseEntity<UserAddressResponseDto> updateAddress(
        ...
        UserAddressResponseDto responseDto = userAddressService.update(userId, addressId, request);
        return ResponseEntity.ok(responseDto);
    }

    //DELETE /{addressId}: 특정 주소 삭제 (204 No Content)
    @DeleteMapping("/{addressId}")
    public ResponseEntity<UserAddressResponseDto> deleteAddress(
            @RequestHeader(USER_ID_HEADER) Long userId,
            @PathVariable Long addressId
    ) {
