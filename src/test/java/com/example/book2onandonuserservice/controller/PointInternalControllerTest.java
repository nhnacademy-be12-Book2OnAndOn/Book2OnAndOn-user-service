...
class PointInternalControllerTest {
    ...
    // 프로젝트에서 WebMvcTest 로딩 시 Redis 관련 Bean이 필요해서 넣어둔 것으로 보임.
    // 실제로 필요 없다면 제거 가능.
    ...
    void earnReviewPoint_ok_andMapsUserId() throws Exception {
        ...
        ArgumentCaptor<EarnReviewPointRequestDto> captor = forClass(EarnReviewPointRequestDto.class);
    ...
    @Test
    @DisplayName("[POST] /internal/users/{userId}/points/earn/order - 내부 주문 적립: 컨트롤러 매핑 필드(userId, orderId, pureAmount)만 검증")
    void earnOrderPoint_ok_andMapsFields() throws Exception {
        ...
        ArgumentCaptor<EarnOrderPointRequestDto> captor = forClass(EarnOrderPointRequestDto.class);
    ...
    @Test
    @DisplayName("[POST] /internal/users/{userId}/points/use - 내부 포인트 사용: 컨트롤러 매핑 필드(userId, orderId, useAmount)만 검증")
    void usePoint_ok_andMapsFields() throws Exception {
        ...
        ArgumentCaptor<UsePointRequestDto> captor = forClass(UsePointRequestDto.class);
    ...
    void refundPoint_ok_andMapsFields() throws Exception {
        ...
        ArgumentCaptor<RefundPointRequestDto> captor = forClass(RefundPointRequestDto.class);
